...
class Cart(models.Model):
    user = models.ForeignKey(
        User, verbose_name="User", related_name="carts", on_delete=models.CASCADE
    )
    ...
    # Creating Model Property to calculate the overall total price of cart
    @property
    def overall_product_price_of_cart(self):
        return sum(
            cart_item.quantity * cart_item.product.price
            for cart_item in self.user.carts.all()
        )

    # Creating Model Property to calculate the total Cart Quantity
    @property
    def total_cart_quantity(self):
        # Assuming there's a related_name "carts" in the User model for the cart items
        return (
            self.user.carts.aggregate(total_quantity=models.Sum("quantity"))[
                "total_quantity"
            ]
            or 0
        )
...
class Transactions(models.Model):
    ...
    provider = models.CharField(choices=PROVIDER_CHOICES, max_length=50, default="SSL")
    status = models.CharField(
        choices=TRANSACTION_STATUS, max_length=50, default="PENDING"
    )
